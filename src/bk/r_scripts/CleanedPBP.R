...
# Clean New PBP Data
pbp21<- fread('/Users/am/Desktop/NFLSets/PBP/NFLPBPNEW2021.csv', sep = ',', header = T, quote="\"", stringsAsFactors = F)
...
pbp21<-merge(pbp21,Schedule,by=c("game_id"),all.x=TRUE)
pbp21$roof.x<-pbp21$roof.y
pbp21$surface.x<-pbp21$surface.y
pbp21<-pbp21[,-c(373,374)]
names(pbp21)[names(pbp21) == "roof.x"] <- "roof"
names(pbp21)[names(pbp21) == "surface.x"] <- "surface"

### Remove Clock Plays###
pbp21<-subset(pbp21,pbp21$qb_spike==0)
pbp21<-subset(pbp21,pbp21$qb_kneel==0)
